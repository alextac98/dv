...
fn as_obj<'a>(v: &'a Value, ctx: &str) -> &'a serde_json::Map<String, Value> {
    v.as_object()
        .unwrap_or_else(|| panic!("{}: expected object", ctx))
}

fn as_arr<'a>(v: &'a Value, ctx: &str) -> &'a [Value] {
    v.as_array()
        .map(|a| a.as_slice())
        .unwrap_or_else(|| panic!("{}: expected array", ctx))
}

fn get_str(obj: &serde_json::Map<String, Value>, key: &str, ctx: &str) -> String {
    obj.get(key)
        .and_then(Value::as_str)
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| panic!("{}: missing or invalid string key '{}'", ctx, key))
}

fn get_num(obj: &serde_json::Map<String, Value>, key: &str, ctx: &str) -> f64 {
    obj.get(key)
        .and_then(Value::as_f64)
        .unwrap_or_else(|| panic!("{}: missing or invalid number key '{}'", ctx, key))
}

fn get_bool(obj: &serde_json::Map<String, Value>, key: &str, ctx: &str) -> bool {
    obj.get(key)
        .and_then(Value::as_bool)
        .unwrap_or_else(|| panic!("{}: missing or invalid bool key '{}'", ctx, key))
}
...
fn assert_expected_dv(case_id: &str, expect: &serde_json::Map<String, Value>, dv: &Dv, abs_tol: f64, rel_tol: f64) {
    let exp_value = get_num(expect, "value", case_id);
    let exp_unit = get_str(expect, "unit", case_id);
    let actual = if exp_unit.is_empty() {
        dv.value()
    } else {
        dv.value_in(&exp_unit)
...
fn assert_expected_scalar(case_id: &str, expect: &serde_json::Map<String, Value>, actual: f64, abs_tol: f64, rel_tol: f64) {
    let exp_value = get_num(expect, "value", case_id);
    let exp_unit = get_str(expect, "unit", case_id);
...
fn build_input_dv(case_obj: &serde_json::Map<String, Value>, case_id: &str) -> Dv {
...
fn run_vectors(file_name: &str) {
    ...
    let root: Value = serde_json::from_str(&raw).expect("invalid JSON in vector file");
    let root_obj = as_obj(&root, "root");

    let defaults = root_obj.get("defaults").map(|v| as_obj(v, "defaults"));
    ...
    for case_v in cases {
        ...
        match op.as_str() {
            ...
            "powf" => {
                ...
                if ok {
                    let result = dv.powf(exponent).unwrap_or_else(|e| panic!("{}: powf failed: {}", case_id, e));
                    assert_expected_dv(&case_id, expect_obj, &result, abs_tol, rel_tol);
                } else {
            ...
            "sqrt" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.sqrt().unwrap_or_else(|e| panic!("{}: sqrt failed: {}", case_id, e));
                    assert_expected_dv(&case_id, expect_obj, &result, abs_tol, rel_tol);
                } else {
            ...
            "ln" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.ln().unwrap_or_else(|e| panic!("{}: ln failed: {}", case_id, e));
            ...
            "log2" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.log2().unwrap_or_else(|e| panic!("{}: log2 failed: {}", case_id, e));
                    assert_expected_scalar(&case_id, expect_obj, result, abs_tol, rel_tol);
                } else {
            ...
            "log10" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.log10().unwrap_or_else(|e| panic!("{}: log10 failed: {}", case_id, e));
                    assert_expected_scalar(&case_id, expect_obj, result, abs_tol, rel_tol);
                } else {
            ...
            "sin" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.sin().unwrap_or_else(|e| panic!("{}: sin failed: {}", case_id, e));
                    assert_expected_scalar(&case_id, expect_obj, result, abs_tol, rel_tol);
                } else {
            ...
            "cos" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.cos().unwrap_or_else(|e| panic!("{}: cos failed: {}", case_id, e));
                    assert_expected_scalar(&case_id, expect_obj, result, abs_tol, rel_tol);
                } else {
            ...
            "tan" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.tan().unwrap_or_else(|e| panic!("{}: tan failed: {}", case_id, e));
                    assert_expected_scalar(&case_id, expect_obj, result, abs_tol, rel_tol);
                } else {
            ...
            "asin" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.asin().unwrap_or_else(|e| panic!("{}: asin failed: {}", case_id, e));
                    assert_expected_dv(&case_id, expect_obj, &result, abs_tol, rel_tol);
                } else {
            ...
            "acos" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.acos().unwrap_or_else(|e| panic!("{}: acos failed: {}", case_id, e));
                    assert_expected_dv(&case_id, expect_obj, &result, abs_tol, rel_tol);
                } else {
            ...
            "atan" => {
                let dv = build_input_dv(case, &case_id);
                if ok {
                    let result = dv.atan().unwrap_or_else(|e| panic!("{}: atan failed: {}", case_id, e));
                    assert_expected_dv(&case_id, expect_obj, &result, abs_tol, rel_tol);
                } else {
...
#[test]
fn regression_units_vectors() {
    run_vectors("units_tests.json");
}

#[test]
fn regression_math_vectors() {

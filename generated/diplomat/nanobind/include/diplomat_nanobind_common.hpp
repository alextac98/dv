...
namespace nanobind::detail
{
    ...
    struct type_caster<dv_pyo3::diplomat::result<T, E>>
    {
        ...
        using Value = dv_pyo3::diplomat::result<T, E>;
        ...
        static handle from_cpp(dv_pyo3::diplomat::result<T, E> value, rv_policy p, cleanup_list *cl) noexcept
        {
            if (value.is_ok()) {
        ...
        operator Value() { 
            if (is_ok) {
                return dv_pyo3::diplomat::Ok<T>(forward_like_<U>(ok_val.value()));
            } else {
                return dv_pyo3::diplomat::Err<E>(forward_like_<V>(err_val.value()));
            }
        }
    ...
    class type_caster<dv_pyo3::diplomat::span<T, E>> {
    ...
    public:
        using Value = dv_pyo3::diplomat::span<T, E>;
        Value value = dv_pyo3::diplomat::span<T, E>();
        ...
        bool from_python(handle src, uint8_t flags, cleanup_list* cleanup) noexcept {
            ...
            if constexpr (is_ndarray_scalar_v<T>) {
                ...
                if (caster.from_python(src, local_flags, cleanup)) {
                    ...
                    value = dv_pyo3::diplomat::span<T, E>(caster.value.data(), caster.value.shape(0));
                    return true;
                }
            }
            ...
            if (nb::inst_check(src) && nb::isinstance<U>(src)) {
                ...
                value = dv_pyo3::diplomat::span<T, E>(reinterpret_cast<T*>(bound_vec->data()), bound_vec->size());
                return true;
            }
            ...
            if (std::is_const_v<T> &&
                ...
                ListCaster caster;
                if (caster.from_python(src, local_flags, cleanup)) {
                    value = dv_pyo3::diplomat::span<T, E>(reinterpret_cast<T*>(caster.value.data()), caster.value.size());
        ...
        static handle from_cpp(dv_pyo3::diplomat::span<T, E> src, rv_policy policy, cleanup_list* cleanup) {
    ...
    template <>
    struct type_caster<dv_pyo3::diplomat::string_view_for_slice> {
        NB_TYPE_CASTER(dv_pyo3::diplomat::string_view_for_slice, const_name("str"))
        ...
        bool from_python(handle src, uint8_t, cleanup_list *) noexcept {
            ...
            value = dv_pyo3::diplomat::string_view_for_slice(str, (size_t) size);
            return true;
        }

        static handle from_cpp(dv_pyo3::diplomat::string_view_for_slice value, rv_policy, cleanup_list *) noexcept {
